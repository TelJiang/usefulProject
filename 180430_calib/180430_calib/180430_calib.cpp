// 180430_calib.cpp : 定义控制台应用程序的入口点。
//
...
int main(int argc, char** argv)
{
	...
	ofstream intrinsicfile("intrinsics_genius.txt");
	ofstream disfile("dis_coeff_genius.txt");
	int num = 0;
	bool bCalib = false;
	...
	cout << distortion_coeff.rows << endl;
	for (int i = 0; i < 3; ++i)
	{
		for (int j = 0; j < 3; ++j)
		{
	...
	while (true)
	{
		...
		Mat src,_corrected;
		cap >> src;
		remap(src, _corrected, mapx, mapy, INTER_LINEAR, BORDER_TRANSPARENT);
		imshow("src", src);
		imshow("cjcjcjc", _corrected);
